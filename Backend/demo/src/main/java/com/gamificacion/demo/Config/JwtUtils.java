...
public class JwtUtils {
	...
	public String generateToken(Map<String, Object> claims, UserDetails usuario) {
		return createToken(claims, usuario);
	}
	...
	private String createToken(Map<String, Object> claims, UserDetails usuario) {
		return Jwts.builder().setClaims(claims).setSubject(usuario.getUsername()).
				claim("authorities", usuario).
				setIssuedAt(new Date(System.currentTimeMillis())).
				setExpiration(new Date(System.currentTimeMillis() + TimeUnit.HOURS.toMillis(24))).
				signWith(SignatureAlgorithm.HS256,jwtStringKey).compact();
	}
	
	public Boolean isTokenValid(String token, UserDetails usuario) {
		final String correo = extraerCorreo(token);
		return correo.equals(usuario.getUsername()) && !isTokenExpired(token);
	}
