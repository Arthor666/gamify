...
@Component({
  ...
  templateUrl: '../html/UsuarioRecompensaAdmin.component.html',
  styleUrls: ['../css/UsuarioRecompensaAdmin.component.css'],
  animations: [
...
export class UsuarioRecompensaAdminComponent implements OnInit {
  ...
  ngOnInit() {
    this.usuarioRecompensaService.getAll().subscribe(data => this.iniciarPaginacion(data));
  ...
  expandir(element: any): void {
    ...
    if (this.statusList.length <= 0) {
      this.statusService.getByClase("recompensaAdmin").subscribe(data => this.statusList = data);
    }
  }
